    ...
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False):
        ...
        if self.train:
            ...
            for i in range(10):
                ...
                train_labels_class.append(
                    torch.index_select(self.train_labels, 0, indices))
                train_data_class.append(
                    torch.index_select(self.train_data, 0, indices))

            # generate balanced pairs
            self.train_data = []
            self.train_labels = []
            # store the amount of each number image
            lengths = [x.shape[0] for x in train_labels_class]
            for i in range(10):
                for j in range(500):  # create 500 pairs
                    # choose random class that is not the same class
                    rnd_cls = random.randint(0, 8)
                    if rnd_cls >= i:
                        rnd_cls = rnd_cls + 1
            ...
            for i in range(10):
                ...
                test_labels_class.append(
                    torch.index_select(self.test_labels, 0, indices))
                test_data_class.append(
                    torch.index_select(self.test_data, 0, indices))
...
def main():
    ...
    if do_learn:  # training mode
        train_loader = torch.utils.data.DataLoader(BalancedMNISTPair(
            '.', train=True, download=True, transform=trans), batch_size=batch_size, shuffle=True)
        test_loader = torch.utils.data.DataLoader(BalancedMNISTPair(
            '.', train=False, download=True, transform=trans), batch_size=batch_size, shuffle=False)
        ...
        prediction_loader = torch.utils.data.DataLoader(BalancedMNISTPair(
            '.', train=False, download=True, transform=trans), batch_size=1, shuffle=True)
#         model.load_state_dict(torch.load(load_model_path))
        model = torch.load(load_model_path)
        data = []
#         data.extend(next(iter(prediction_loader))[0][:3:2]) # this line tests different number
        # this line tests the same number
        data.extend(next(iter(prediction_loader))[0][:2])
...
if __name__ == '__main__':
    # To test
    # Edit the `do_learn = True` as `False` to test
    main()
